...
begin
    if (LAYER == 0) begin
        for (addr = 0; addr < MEM_DEPTH; addr = addr + 1) begin
            bank0[addr] = {64'h407f40000000000};
            bank1[addr] = {64'h407f40000000000};
            bank2[addr] = {64'h407f40000000000};
            bank3[addr] = {64'h407f40000000000}; // 500
            // just for debug, in reality it need to be zero
        end
