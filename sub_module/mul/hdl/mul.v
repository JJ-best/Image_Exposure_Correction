...
wire hidden_inA_lo = |in_A[(pFP_WIDTH-2) : pMANTISSA_WIDTH];
wire hidden_inB_lo = |in_B[(pFP_WIDTH-2) : pMANTISSA_WIDTH];
wire hidden_inA_hi = |in_A[(pDATA_WIDTH-2):(pDATA_WIDTH-pEXP_WIDTH-1)];
wire hidden_inB_hi = |in_B[(pDATA_WIDTH-2):(pDATA_WIDTH-pEXP_WIDTH-1)];
...
assign array_in_C1       = (mode[1] == C_MUL )? {{(pFP_WIDTH - pMANTISSA_WIDTH -1 ){1'b0}} , hidden_a_re      , a_re_r    [(pMANTISSA_WIDTH-1):0]} 
                                                : { {(pFP_WIDTH - pMANTISSA_WIDTH -1 ){1'b0}} , hidden_inB_hi , in_B[(pDATA_WIDTH-pFP_WIDTH+pMANTISSA_WIDTH-1):(pDATA_WIDTH-pFP_WIDTH)]};
...
wire [pFP_WIDTH-1:0] a_hi_word = in_A[(pDATA_WIDTH-1) : pFP_WIDTH];
wire [pFP_WIDTH-1:0] a_lo_word = in_A[(pFP_WIDTH-1)  : 0];
wire [pFP_WIDTH-1:0] b_hi_word = in_B[(pDATA_WIDTH-1) : pFP_WIDTH];
wire [pFP_WIDTH-1:0] b_lo_word = in_B[(pFP_WIDTH-1)  : 0];

wire complex_mode = (mode[1] == C_MUL);
wire float_mode   = (mode[1] == INT_MUL); // reuse mode[1]==1 as float mul

wire [pEXP_WIDTH-1:0] exp_a_lo = a_lo_word[(pFP_WIDTH-2) : pMANTISSA_WIDTH];
wire [pEXP_WIDTH-1:0] exp_b_lo = b_lo_word[(pFP_WIDTH-2) : pMANTISSA_WIDTH];
wire [pEXP_WIDTH-1:0] exp_a_hi = a_hi_word[(pFP_WIDTH-2) : pMANTISSA_WIDTH];
wire [pEXP_WIDTH-1:0] exp_b_hi = b_hi_word[(pFP_WIDTH-2) : pMANTISSA_WIDTH];

wire [pMANTISSA_WIDTH-1:0] mant_a_lo = a_lo_word[(pMANTISSA_WIDTH-1):0];
wire [pMANTISSA_WIDTH-1:0] mant_b_lo = b_lo_word[(pMANTISSA_WIDTH-1):0];
wire [pMANTISSA_WIDTH-1:0] mant_a_hi = a_hi_word[(pMANTISSA_WIDTH-1):0];
wire [pMANTISSA_WIDTH-1:0] mant_b_hi = b_hi_word[(pMANTISSA_WIDTH-1):0];

assign exp_A0 = (mode[1] == C_MUL)? br_add_bi [(pMANTISSA_WIDTH + pEXP_WIDTH - 1) :pMANTISSA_WIDTH] : {pEXP_WIDTH{1'b0}};    // * exp of (b_re + b_im)
assign exp_A1 = (mode[1] == C_MUL)? a_im_r    [(pMANTISSA_WIDTH + pEXP_WIDTH - 1) :pMANTISSA_WIDTH] : {pEXP_WIDTH{1'b0}};    // * exp of a_im
assign exp_B0 = (mode[1] == C_MUL)? ar_sub_ai [(pMANTISSA_WIDTH + pEXP_WIDTH - 1) :pMANTISSA_WIDTH] : exp_a_lo; // imagernary part of aim;    // * exp of (a_re - a_im)
assign exp_B1 = (mode[1] == C_MUL)? b_im_r    [(pMANTISSA_WIDTH + pEXP_WIDTH - 1) :pMANTISSA_WIDTH] : exp_b_lo; // imagernary part of bim;    // * exp of b_im              
assign exp_C0 = (mode[1] == C_MUL)? br_sub_bi [(pMANTISSA_WIDTH + pEXP_WIDTH - 1) :pMANTISSA_WIDTH] : exp_a_hi; // real part of are;    // * exp of (b_re - b_im)
assign exp_C1 = (mode[1] == C_MUL)? a_re_r    [(pMANTISSA_WIDTH + pEXP_WIDTH - 1) :pMANTISSA_WIDTH] : exp_b_hi; // real part of bre;    // * exp of a_re

assign NaN_a  = (mode[1] == C_MUL)? ( ((& exp_A0) & (|br_add_bi [(pMANTISSA_WIDTH-1):0])) | ( (& exp_A1) & (|a_im_r [(pMANTISSA_WIDTH-1):0])) )
                                  : 1'b0;
assign NaN_b  = (mode[1] == C_MUL)? ( ((& exp_B0) & (|ar_sub_ai [(pMANTISSA_WIDTH-1):0])) | ( (& exp_B1) & (|b_im_r [(pMANTISSA_WIDTH-1):0])) )
                                  : ( ((& exp_B0) & (|mant_a_lo)) | ((& exp_B1) & (|mant_b_lo)) );
assign NaN_c  = (mode[1] == C_MUL)? ( ((& exp_C0) & (|br_sub_bi [(pMANTISSA_WIDTH-1):0])) | ( (& exp_C1) & (|a_re_r [(pMANTISSA_WIDTH-1):0])) )
                                  : ( ((& exp_C0) & (|mant_a_hi)) | ((& exp_C1) & (|mant_b_hi)) );

// The in_valid use in_valid if mode[1] == INT_MUL
wire exp_valid_B = (mode[1] == INT_MUL)? in_valid : fp_add_ready[1];
wire exp_valid_C = (mode[1] == INT_MUL)? in_valid : fp_add_ready[2];
...
fmul_exp  exponent_op_B( .clk( clk ) , .rst_n( rst_n ), .in_valid( exp_valid_B ), .exp_A( exp_B0 ), .exp_B( exp_B1 ),  .exp_o( exp_B_out ), .out_inf( inf_B ) , .out_valid( exp_ready_B ));
fmul_exp  exponent_op_C( .clk( clk ) , .rst_n( rst_n ), .in_valid( exp_valid_C ), .exp_A( exp_C0 ), .exp_B( exp_C1 ),  .exp_o( exp_C_out ), .out_inf( inf_C ) , .out_valid( exp_ready_C ));

// The sign_B = aim_sign ^ bim_sign sign_C = are_sign ^ bre_sign
assign sign_A = ((|array_in_A0) && (|array_in_A1))? (br_add_bi[pFP_WIDTH-1] ^ a_im_r[pFP_WIDTH-1]) : 1'b0;  // * zero case while doing fp_mul
assign sign_B = (mode[1] == C_MUL)? ( ((|array_in_B0) && (|array_in_B1))? (ar_sub_ai[pFP_WIDTH-1] ^ b_im_r[pFP_WIDTH-1]) : 1'b0 )
                                   : ( ((|array_in_B0) && (|array_in_B1))? (in_A[pFP_WIDTH-1] ^ in_B[pFP_WIDTH-1]) : 1'b0 );
assign sign_C = (mode[1] == C_MUL)? ( ((|array_in_C0) && (|array_in_C1))? (br_sub_bi[pFP_WIDTH-1] ^ a_re_r[pFP_WIDTH-1]) : 1'b0 )
                                   : ( ((|array_in_C0) && (|array_in_C1))? (in_A[pDATA_WIDTH-1] ^ in_B[pDATA_WIDTH-1]) : 1'b0 );
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
wire fp_sign_A = (|({exp_A_rounded , frac_A_rounded }))? sign_A_reg[EXP_OP_LATENCY + ROUNDER_LATENCY -1] : 1'b0;
wire fp_sign_B = (|({exp_B_rounded , frac_B_rounded }))? sign_B_reg[EXP_OP_LATENCY + ROUNDER_LATENCY -1] : 1'b0;
wire fp_sign_C = (|({exp_C_rounded , frac_C_rounded }))? sign_C_reg[EXP_OP_LATENCY + ROUNDER_LATENCY -1] : 1'b0;
assign FP_num_A = (NaN_a_reg[EXP_OP_LATENCY + ROUNDER_LATENCY -1])? {NaN_num} : {fp_sign_A , exp_A_rounded , frac_A_rounded };  // num_A = a_im * (b_re + b_im)
assign FP_num_B = (NaN_b_reg[EXP_OP_LATENCY + ROUNDER_LATENCY -1])? {NaN_num} : {fp_sign_B , exp_B_rounded , frac_B_rounded };  // num_B = b_im * (a_re - a_im)
assign FP_num_c = (NaN_c_reg[EXP_OP_LATENCY + ROUNDER_LATENCY -1])? {NaN_num} : {fp_sign_C , exp_C_rounded , frac_C_rounded };  // num_C = a_re * (b_re - b_im)


wire fp_mul_valid = (rounder_ready_B || rounder_ready_C) && (mode[1] == INT_MUL);
...
assign result_c     = (mode[1] == C_MUL)? { y_re , y_im} : {FP_num_c, FP_num_B}; 
...
assign out_valid    = (cmul_im_ready & cmul_re_ready) || fp_mul_valid;
