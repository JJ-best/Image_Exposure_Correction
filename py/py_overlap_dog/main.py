...
from fusion import run_fusion, save_debug_dat
import numpy as np
...
if __name__ == "__main__":
    ...
    out_dir_dat_debug = root / "imgs_dat_output"
    out_dir_dat_debug.mkdir(parents=True, exist_ok=True)
    ...
    # 3. Fusion
    fused_out_bmp = out_dir_fusion / "final_fused_image.bmp"
    Image.fromarray((fused_full_img * 255).astype(np.uint8), mode="RGB").save(fused_out_bmp)

    save_debug_dat(fused_full_img, "final_fused_image", out_dir_fusion)

    print(f"All done! Fusion result saved to: {fused_out_bmp}")
